...
def handle_critic_verdict(nid: str, result, graph, recovered_branches: dict,
    ...
    # Resolve the original (root) target node by tracing back parent planners
    root_target = target_nid
    current = target_nid
    while current:
        parent_planner = next((p for p in graph.g.predecessors(current) 
                               if graph.g.nodes[p].get("skill") == "planner"), None)
        if parent_planner:
            ancestor = graph.g.nodes[parent_planner].get("metadata", {}).get("recovers")
            if ancestor:
                root_target = ancestor
                current = ancestor
                continue
        break

    if root_target and not recovered_branches.get(root_target):
        recovered_branches[root_target] = True
        ...
        cap_hit.append(root_target)
        print(f"  ↪ critic-fail on {root_target} already recovered once; "
