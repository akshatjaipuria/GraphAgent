...
fetch_url:         trafilatura. Clean text fetching.
...
async def _trafilatura_fetch(url: str, timeout: int = 20) -> dict:
    import trafilatura
    headers = {
        "User-Agent": "WikiTextReader/1.0 (bot@example.com; research project crawler)"
    }
    ...
    if r.status_code != 200:
        return {
            ...
            "length_bytes": len(r.text.encode("utf-8")),
            "text": f"Error HTTP {r.status_code}: {r.text[:200]}",
        }
...
async def fetch_url(url: str, timeout: int = 20) -> dict:
    """Fetch clean text from a URL via trafilatura. Example: fetch_url("https://example.com")."""
    return await _trafilatura_fetch(url, timeout)
